...
merge_subspp=1
last_seen_info=0
sp_col_name="Alternate name"
last_seen_col_name="species codes"
file_name='test_sp_correction.xls'
Sheet_name="Sheet2"
#END USER INPUT
...
for cell in sheet.row(0): 
	if sp_col_name in cell.value:
		sp_col=i
		break
	i=i+1
Uncorrected_sp_names=sheet.col(sp_col)

i=0
for cell in sheet.row(0): 
	if last_seen_col_name.decode('utf-8','replace') in cell.value:
		last_seen_col=i
		break
	i=i+1
Last_seen_all=sheet.col(sp_col)
...
#CREATE OUTPUT TABLE
Spp_data=[]
Sp_data=[''sp_name', 'ID_error', 'JP_code', 'lastseen']
