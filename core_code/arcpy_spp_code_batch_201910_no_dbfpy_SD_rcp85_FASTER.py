...
#python D:\Dropbox\code\arcpyVAtool\core_code\arcpy_spp_code_batch_RD72_no_dbfpy_SD_rcp45.py
#if running parallel, must run this from cmd with script above, python must be on system path
#USER INPUT

island="all" #
rootdir=r"E:/VA2_rerun/output/SD85/" #location for outputs. ?whichever is data dir, will have to have subfolders: results/, results/la/, la/ (where you place CCE and FCE files)
...
landscape_factor_dir=r"E:/VA2_rerun/data/landscape/" #whichever is data dir,will have to have subfolders: gaplandcover/ (where gaplandcov_hi is placed)
...
ce_data_dir=r"E:/VA2_rerun/data/CEs V2_2019/rcp85/" #location of climate envelope files "Y:/PICCC_data/VA data/CEs_500m/"
#ce_data_dir=r"E:/VA2_rerun/data/CEs V2_2016/SD RCP45 CEs V2 range maps archipelago reprojected 2bits/" #location of climate envelope files "Y:/PICCC_data/VA data/CEs_500m/"
...
try:
    ...
    if pre_process_envelopes:
        ...
        if arcpy.Exists(loc_COR_FCE)==True and overwrite==0:
            print 'pre_process_envelopes already done for all spp'
        else:
            i=0 #for debug
                ...
                if int(arcpy.GetCount_management(sp_CO_points_loc).getOutput(0))==0 and use_bio_region_filter==1:
                    ...
                    if arcpy.Exists(loc_COR_FCE)==False or overwrite==1:
                        if island=='all':
                            ...
                            inRaster=arcpy.Raster(inRaster)
                            if inRaster.maximum==None or str(inRaster.maximum)=='-inf':
                                FCE_full=inRaster
                            else:
                                if get_num_attributes(inRaster, "MINIMUM")==1 and get_num_attributes(inRaster, "MAXIMUM")==1:
                                    FCE_full=inRaster
    ...
    if calculate_veg_type_areas_current:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calculate_veg_type_areas_current already done for all spp'
        else:
    ...
    if calculate_veg_type_areas_BPS:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calculate_veg_type_areas_BPS already done for all spp'
        else:
    ...
    if calculate_native_habitat_areas_HIGAP:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calculate_native_habitat_areas_HIGAP already done for all spp'
        else:
    ...
    if calculate_all_habitat_areas_HIGAP:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calculate_all_habitat_areas_HIGAP already done for all spp'
        else:
    ...
    if calculate_native_habitat_areas_LANDFIRE:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calculate_native_habitat_areas_LANDFIRE already done for all spp'
        else:
    ...
    if calculate_all_habitat_areas_LANDFIRE:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calculate_all_habitat_areas_LANDFIRE already done for all spp'
        else:
    ...
    if calculate_native_habitat_areas_BPS_LANDFIRE:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calculate_native_habitat_areas_BPS_LANDFIRE already done for all spp'
        else:
    ...
    if calculate_change_in_n_available_habitat:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calculate_change_in_n_available_habitat already done for all spp'
        else:
    ...
    if calc_cce_total_area:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_cce_total_area already done for all spp'
        else:
    ...
    if calc_fce_total_area:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_fce_total_area already done for all spp'
        else:
    ...
    if calc_cce_fce_dif:
        ...
        resultsdir=resultsdir0+sp_code_st+"/"
        sp_code=str(int(sp_code_st)) #get rid of extra zeros
        opath="%sDBFs/area_diff%s.csv" %(resultsdir,sp_code_st)
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_cce_fce_dif already done for all spp'
        else:
    ...
    if map_tol_zone:
        ...
        resultsdir=resultsdir0+sp_code_st+"/"
        sp_code=str(int(sp_code_st)) #get rid of extra zeros
        opath=r"%sintersect_%s.tif" %(resultsdir,sp_code_st)
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'map_tol_zone already done for all spp'
        else:
    ...
    if map_mrf_zone:
        ...
        resultsdir=resultsdir0+sp_code_st+"/"
        sp_code=str(int(sp_code_st)) #get rid of extra zeros
        opath=r"%sMRFzn_%s.tif" %(resultsdir,sp_code_st)
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'map_mrf_zone already done for all spp'
        else:
    ...
    if map_mig_zone_pt1:
        ...
        resultsdir=resultsdir0+sp_code_st+"/"
        sp_code=str(int(sp_code_st)) #get rid of extra zeros
        opath=r"%sMigrate_%s.tif" %(resultsdir,sp_code_st)
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'map_mig_zone_pt1 already done for all spp'
        else:
    ...
    if calc_mean_elev_cce:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_mean_elev_cce already done for all spp'
        else:
    ...
    if calc_mean_elev_fce:
        ...
        resultsdir=resultsdir0+sp_code_st+"/"
        sp_code=str(int(sp_code_st)) #get rid of extra zeros
        opath="%sDBFs/max_elev_zone_vals_%s.csv" %(resultsdir,sp_code_st)
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_mean_elev_fce already done for all spp'
        else:
    ...
    if count_cce_bioreg:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'count_cce_bioreg already done for all spp'
        else:
    ...
    if count_cce_bioreg_transition_areas:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'count_cce_bioreg_transition_areas already done for all spp'
        else:
    ...
    if calc_cce_precip_interannual_var:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_cce_precip_interannual_var already done for all spp'
        else:
    ...
    if create_rep_zones:
        ...
        resultsdir=resultsdir0+sp_code_st+"/"
        sp_code=str(int(sp_code_st)) #get rid of extra zeros
        opath=r"%sresponse_zone_%s.tif" %(resultsdir,sp_code_st)
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'create_rep_zones already done for all spp'
        else:
    ...
    if calc_resp_zone_area:
        ...
        resultsdir=resultsdir0+sp_code_st+"/"
        sp_code=str(int(sp_code_st)) #get rid of extra zeros
        opath=r"%sDBFs/%s_zone_area.csv" %(resultsdir, sp_code_st)
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_resp_zone_area already done for all spp'
        else:
    ...
    if calc_zone_slr_area:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_zone_slr_area already done for all spp'
        else:
    ...
    if calc_zone_lava_flow_area:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_zone_lava_flow_area already done for all spp'
        else:
    ...
    if calc_zone_hab_qual:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_zone_hab_qual already done for all spp'
        else:
    ...
    if calc_eff_hab_qual_nonpioneer:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_eff_hab_qual_nonpioneer already done for all spp'
        else:
    ...
    if calc_eff_hab_qual_pioneer:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_eff_hab_qual_pioneer already done for all spp'
        else:
    ...
    if chose_eff_hab_qual:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'chose_eff_hab_qual already done for all spp'
        else:
    ...
    if calc_hab_qual:
        ...
        resultsdir=resultsdir0+sp_code_st+"/"
        sp_code=str(int(sp_code_st)) #get rid of extra zeros
        opath="%sGBU_%s.tif" %(resultsdir, sp_code_st)
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_hab_qual already done for all spp'
        else:
    ...
    if calc_fragmentation:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_fragmentation already done for all spp'
        else:
    ...
    if calc_dist_to_top_of_island:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_dist_to_top_of_island already done for all spp'
        else:
    ...
    if calc_protected_area:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_protected_area already done for all spp'
        else:
    ...
    if calc_ung_free_area:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_ung_free_area already done for all spp'
        else:
    ...
    if calc_slope_metrics:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_slope_metrics already done for all spp'
        else:
    ...
    if calc_zone_topo_complexity:
        ...
        resultsdir=resultsdir0+sp_code_st+"/"
        sp_code=str(int(sp_code_st)) #get rid of extra zeros
        opath=r"%sDBFs/%s_zone_topo_complexity.csv" %(resultsdir, sp_code_st)
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_zone_topo_complexity already done for all spp'
        else:
...
##                pool.join()
    ...
    if calc_ppt_gradient:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_ppt_gradient already done for all spp'
        else:
    ...
    if calc_zone_invasibility:
        ...
        if arcpy.Exists(opath)==True and overwrite==0:
            print 'calc_zone_invasibility already done for all spp'
        else:
